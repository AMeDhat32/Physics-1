...
// 1111.110
	...
	string decToHex(double dec) {
		...
		while (integral) {
			int rem = integral % 16;
			if (rem < 10) hex = to_string(rem) + hex;
		...
		for (int i = 0; i < 10; i++) {
			...
			fractional -= rem;
			if (rem < 10) hex = hex + to_string(rem);
			else hex = hex + string(1, 'A' + rem - 10);
		}
		return hex;
	}
	...
	double hexToDec(string hex) {
		...
		// integer part
		for (int i = dec_point - 1; i >= 0; i--) {
			if (hex[i] >= '0' and hex[i] <= '9') dec = dec + (hex[i] - '0') * base;
			else dec = dec + (hex[i] - 'A' + 10) * base;
			base *= 16;
		}
